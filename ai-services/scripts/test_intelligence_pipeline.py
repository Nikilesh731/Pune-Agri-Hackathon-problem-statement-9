...
def test_intelligence_pipeline():
    ...
    print("ðŸ§  AI Intelligence Pipeline Test")
    print("=" * 60)
...
"""
    ...
    print("ðŸ“„ Test Document: Complex Scheme Application")
    print("-" * 40)
    
    # Test with scheme_application type
    result = extraction_service.extract_document(test_document, "scheme_application")
    
    print(f"âœ… Extraction Success: {bool(result.get('structured_data'))}")
    print(f"ðŸ“Š Extraction Confidence: {result.get('confidence', 0):.3f}")
    print(f"ðŸ“ Structured Fields: {len(result.get('structured_data', {}))}")
    print(f"ðŸ§  Reasoning Items: {len(result.get('reasoning', []))}")
    print(f"ðŸ” Reasoning Insights: {len(result.get('reasoning_insights', []))}")
    
    # Display key extracted fields
    print("\nðŸ“‹ Key Extracted Fields:")
    ...
    for field in key_fields:
        if field in structured_data:
            print(f"  â€¢ {field}: {structured_data[field]}")
    
    # Display enhanced reasoning
    print("\nðŸ§  Enhanced Reasoning Sample:")
    ...
    for insight_type, messages in insight_types.items():
        print(f"  ðŸ“Š {insight_type}: {len(messages)} insights")
    ...
    if predictions and 'predictions' in predictions:
        ...
        for pred_type, pred_info in pred_data.items():
            print(f"  ðŸ“ˆ {pred_type}: {pred_info.get('value', 'N/A')} (confidence: {pred_info.get('confidence', 0):.2f})")
            reasoning = pred_info.get('reasoning', [])
            if reasoning:
                print(f"    Reasoning: {reasoning[0]}")
    
    # Test semantic extraction capabilities
    print("\nðŸŽ¯ Semantic Extraction Test:")
    semantic_test = """
...
"""
    ...
    print(f"  ðŸ“ Semantic Fields Extracted: {len(semantic_structured)}")
    semantic_keys = ['farmer_name', 'location', 'description', 'mobile_number', 'aadhaar_number']
    for key in semantic_keys:
        if key in semantic_structured:
            print(f"    â€¢ {key}: {semantic_structured[key]}")
    
    # Test field filtering
    print("\nðŸ§¹ Field Filtering Test:")
    filtered_fields = result.get('extracted_fields', {})
    print(f"  ðŸ“Š Total Fields After Filtering: {len(filtered_fields)}")
    ...
    print("  ðŸ“‹ Field Sources:")
    for source, count in sources.items():
        print(f"    â€¢ {source}: {count} fields")
    
    # Summary
    print("\nðŸ“ˆ Intelligence Pipeline Summary:")
    print("=" * 40)
    print(f"âœ… Enhanced Extraction: {len(result.get('structured_data', {}))} fields")
    print(f"ðŸ§  Reasoning Engine: {len(result.get('reasoning_insights', []))} insights")
    print(f"ðŸ”® Predictive Analytics: {len(result.get('predictions', {}).get('predictions', {}))} predictions")
    print(f"ðŸ§¹ Noise Filtering: Applied successfully")
    print(f"ðŸŽ¯ Overall Pipeline: Working with intelligence enhancement")
    
    print("\nðŸŽ‰ AI Intelligence Pipeline Test Complete!")
